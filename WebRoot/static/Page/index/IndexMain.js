Hoo.define('Page.index.IndexMain',{
	...
	listeners : {
	   create : function($page){
			var me = this;
		    var srcs = ['Template/index/Index_tabbar','Template/index/Index_module_x','Template/index/Index_module_mall','Template/index/Index_module_show','Template/index/Index_module_shop','Template/index/Index_module_more'];
		    var modules = ['module-x','module-mall','module-show','module-shop','module-me'];
		    Hoo.util.Array.each(srcs,function(ele,i){ 
		    	Hoo.util.Array.replace(srcs,i,Hoo.Application.getBasePath() + ele);
		    });
		    
			Hoo.util.Ajax.loadTpl(srcs,function(html,xTpl,mallTpl,showTpl,shopTpl,meTpl){
				var index = -1;
				...
				$page.find('.weui_tabbar a').bind('click',function(){
					...
					if(module != ''){
						...
						if(tab.length == 0){
							...
							if(index == 2){ tpl = showTpl; }else
							if(index == 3){ tpl = shopTpl; }else
							if(index == 4){ tpl = meTpl;   }
	   ...
	   beforeShow:function(container,params){
